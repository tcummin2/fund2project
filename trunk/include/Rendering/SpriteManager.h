...
public:
	...
	static int loadFile(std::string input);
	//! Loads a sprite file
	/*! of "DarkFunction Editor" .sprite type
        \param input The filename of the .sprite file */
	static void loadSpriteFile(std::string);

	//! Returns a sprite
	/*! \param input The name of the sprite */
	static AnimatedSprite getSprite(std::string input);
	//! Adds a sprite to the manager
	/*! \param Name the name to store the Sprite by
        \param sprite The sprite to store
    */
	static void addSprite(std::string name, AnimatedSprite sprite);
protected:
    static void loadSpriteDirectory(std::string, rapidxml::xml_node<>*);
private:
